...
export function GettingStartedWidget({ onOpenWorkflow }: GettingStartedWidgetProps) {
    ...
    const steps = [
        ...
        },
        {
            ...
            description: 'Lihat persebaran daerah dengan anomali markup tertinggi di Indonesia pada peta Kop-Mart.',
            icon: Map,
    ...
    // Auto-scroll to the next active step
    useEffect(() => {
        const firstPendingIdx = steps.findIndex(s => !progress[s.id as keyof typeof progress]);
        if (firstPendingIdx !== -1 && scrollContainerRef.current) {
            setTimeout(() => {
                const stepEl = document.getElementById(`onboarding-step-${steps[firstPendingIdx].id}`);
            ...
            }, 300); // Small delay to let React render and shrink completed steps
        }
    }, [progress]);
    ...
    return (
        <Card className="tour-getting-started border-blue-200/50 dark:border-blue-900/30 bg-gradient-to-br from-white to-blue-50/15 dark:from-[#0B1120] dark:to-blue-950/5 p-5 shadow-lg relative overflow-hidden transition-all duration-300">
            ...
            {/* Step list */}
            <div ref={scrollContainerRef} className="relative space-y-4 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                {steps.map((step, idx) => {
