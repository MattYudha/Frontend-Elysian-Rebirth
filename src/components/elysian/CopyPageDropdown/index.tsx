...
import { MarkdownLogo } from "@/components/elysian/logos/markdown";
import { MCPLogo } from "@/components/elysian/logos/mcp";
import { VSCodeLogo } from "@/components/elysian/logos/vscode";
...
export default function CopyPageDropdown() {
  ...
  const connectToCursor = () => {
    ...
    window.open(
      `cursor://anysphere.cursor-deeplink/mcp/install?name=Elysian%20Docs&config=${encodedConfig}`,
      "_blank",
    );
  };
  ...
  const connectToVSCode = () => {
    ...
    const encodedConfig = btoa(JSON.stringify(mcpConfig));
    window.open(
      `vscode://anthropic.claude-code/mcp/install?name=Elysian%20Docs&config=${encodedConfig}`,
      "_blank",
    );
  };
