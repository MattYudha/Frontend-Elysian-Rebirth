...
export default function AgentChatbotComparison(): JSX.Element {
  ...
  const cardInactiveClass = "bg-gray-800 border-gray-700 hover:border-sky-500/50";
  const cardActiveClass = "bg-gray-700 border-sky-500";
  ...
  return (
    <div className="my-8">
        ...
        {systems.map((system) => (
          ...
          >
            <div className="flex items-center justify-between">
              <h4 className="font-medium text-sky-400">{system.name}</h4>
      ...
      {selectedSystem && (
        <div className="border border-sky-500/50 rounded-lg p-5 bg-gray-800/80 animate-fadeIn">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-lg font-semibold text-sky-400">{selectedSystem.name}</h3>
            <span className="text-sm bg-sky-900 text-sky-200 px-3 py-1 rounded-full">
              {selectedSystem.type}
            </span>
          </div>

          <div className="mb-4">
            <h4 className="text-sm font-medium text-sky-300 mb-2">Key Features:</h4>
          ...
          <div className="mb-4">
            <h4 className="text-sm font-medium text-sky-300 mb-2">Capabilities:</h4>
            <div className="flex flex-wrap gap-2">
              {selectedSystem.capabilities.map((capability) => (
                <span
                  key={capability}
                  className="text-xs bg-sky-900/30 text-sky-200 px-2 py-1 rounded-full"
                >
                  {capability}
                </span>
              ))}
          ...
          <div className="mb-4">
            <h4 className="text-sm font-medium text-sky-300 mb-2">Best Used For:</h4>
            <ul className="space-y-1">
              {selectedSystem.bestFor.map((use) => (
                <li key={use} className="text-sm text-gray-300 flex items-center">
                  <span className="text-sky-500 mr-2">•</span> {use}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h4 className="text-sm font-medium text-sky-300 mb-2">Limitations:</h4>
            <ul className="space-y-1">
