    ...
    <div className="my-6 rounded-lg border-2 border-solid border-sky-500 bg-gray-800 p-5 shadow-lg">
      ...
      {currentGoalDetails && selectedGoal !== "unknown" && (
        <div className="mt-6 rounded-md border border-sky-500/50 bg-sky-900/60 p-4 shadow-sm">
          <h5 className="mb-1 font-semibold text-sky-400">{currentGoalDetails.name}</h5>
          <p className="mb-3 text-sm text-sky-100">{currentGoalDetails.description}</p>
          <h6 className="mb-1 text-sm font-medium text-sky-300">
            Recommended LLaMA-Factory Features:
          </h6>
          <ul className="list-inside list-disc space-y-1 text-sm text-sky-100">
            {currentGoalDetails.recommendedFeatures.map((feature) => (
              <li key={feature}>{feature}</li>
            ))}
          </ul>
          <div className="mt-3 rounded bg-sky-950/50 p-2 text-xs text-sky-200">
