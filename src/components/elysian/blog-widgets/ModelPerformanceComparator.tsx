...
const mockResponses: Record<string, { rag: ModelResponse; finetuned: ModelResponse }> = {
  ...
  "How do I implement authentication in the framework?": {
    ...
    finetuned: {
      response:
        "To implement authentication, install @elysian/auth, configure your preferred provider in config.ts, and add the auth middleware to your routes. Full implementation details in docs.",
      latency: 0.9,
      tokens: 134,
    },
  },
...
export default function ModelPerformanceComparator(): JSX.Element {
  ...
  return (
    <div className="border-2 border-solid border-sky-500 rounded-lg p-5 mb-6 bg-gray-800">
      ...
      {responses && (
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div className="bg-sky-900/30 p-3 rounded-lg border border-sky-500/50">
            <h4 className="text-base font-semibold text-sky-400 mb-2">Fine-tuned Response</h4>
            <div className="text-sm text-sky-100 mb-3">{responses.finetuned.response}</div>
