    ...
    <div className="my-5 rounded-lg border-2 border-sky-500 bg-gray-800 p-5 text-gray-100 shadow-lg">
      ...
      <div className="mb-5 flex flex-wrap gap-2.5">
        {componentsData.map((component) => (
          <button
            type="button"
            key={component.id}
            className={`m-1 cursor-pointer rounded-md border border-sky-700 bg-sky-600 py-2.5 px-3.5 text-white transition-colors duration-200 ease-in-out hover:border-sky-800 hover:bg-sky-700 font-medium ${
              selectedComponentId === component.id
                ? "border-sky-900 bg-sky-800 ring-2 ring-sky-500 ring-offset-2 ring-offset-gray-800"
                : ""
            }`}
      ...
      {selectedComponent && (
        <div className="mt-5 rounded-md border border-gray-600 bg-gray-700 p-4 text-gray-200">
          <h4 className="mb-2 mt-0 text-lg text-sky-300">{selectedComponent.name}</h4>
          <p className="leading-relaxed text-gray-200">{selectedComponent.description}</p>
          <h5 className="mt-3 mb-1.5 text-base text-sky-200">Key Details:</h5>
