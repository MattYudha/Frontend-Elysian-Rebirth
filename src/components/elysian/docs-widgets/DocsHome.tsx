...
const codeExample = `import { Elysian, Agent } from "@elysian/core";
...
const quickLinks = [
  {
    title: "Quick Start",
    description: "Get up and running with Elysian in minutes.",
...
export default function DocsHome() {
  return (
    <div className="docs-home -mt-8 max-w-7xl mx-auto md:px-4">
          ...
          {quickLinks.map((link) => (
            ...
            >
              <div className="flex items-start gap-2 md:gap-3">
                ...
                <Link
                  href={link.href}
                  className="text-white font-medium hover:text-sky-400 transition-colors no-underline hover:no-underline"
                >
      ...
      {/* ElysianOps Console Section */}
      <div className="my-8 md:my-12 bg-black">
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-3">
          <div className="flex items-center gap-2">
            <Squares2X2Icon className="w-5 h-5 md:w-6 md:h-6 text-[#00d992]" />
            <span className="text-xl md:text-2xl font-semibold text-white">ElysianOps Console</span>
          </div>
          <span className="px-2 py-0.5 text-xs md:text-sm rounded-full bg-zinc-800/80 text-zinc-300 border border-zinc-700/50">
        ...
        {/*   <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {elysianOpsFeatures.map((feature) => (
            <Link href={feature.href} className="no-underline hover:no-underline">
              <div
                key={feature.title}
