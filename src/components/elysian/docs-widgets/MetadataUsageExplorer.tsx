...
const MetadataUsageExplorer = () => {
  ...
  const [selectedSDK, setSelectedSDK] = useState<"typescript" | "vercel" | "python">("typescript");

  const selectedUsage = metadataUsageData.find((usage) => usage.id === selectedUsageId);

  const sdkOptions: { id: "typescript" | "vercel" | "python"; name: string }[] = [
  ...
  return (
    ...
    >
      ...
      {selectedUsage && (
        ...
        >
          <h4 className="mb-2 mt-0 text-lg text-sky-300">{selectedUsage.name}</h4>
