...
const TagExplorer = () => {
  ...
  const [selectedSDK, setSelectedSDK] = useState<"typescript" | "python" | "vercel">("typescript");

  const selectedExample = tagExamples.find((example) => example.id === selectedExampleId);

  const sdkOptions: { id: "typescript" | "python" | "vercel"; name: string }[] = [
  ...
  return (
    ...
    >
          ...
          <div
            ...
            className={`cursor-pointer rounded-md px-4 py-2 text-sm font-medium transition-colors duration-200 ease-in-out ${
              selectedExampleId === example.id
                ? "bg-sky-400/10 text-sky-400  border-solid border border-sky-400/20 "
                : "border-solid border border-sky-400/20 text-sky-300 hover:bg-sky-800/30 hover:text-sky-100"
            }`}
      ...
      {selectedExample && (
        ...
        >
          <h4 className="mb-2 mt-0 text-lg text-sky-300">{selectedExample.name}</h4>
          ...
          {/* Code Example */}
          <div className="mb-4">
            <h5 className="mb-2 text-base text-sky-200">Example:</h5>
            <CodeBlock
              language={
