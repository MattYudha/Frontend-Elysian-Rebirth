...
export const ExampleCard = ({ example }: ExampleCardProps) => {
  return (
    <Link href={`/examples/agents/${example.slug}/`} className="no-underline">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3 }}
        className="group relative border-solid bg-gradient-to-br from-sky-500/20 via-slate-900/70 to-slate-900/30 border-sky-500/20 border rounded-xl overflow-hidden transition-all duration-300 h-full hover:border-sky-500/40 hover:shadow-2xl hover:shadow-sky-600/20 cursor-pointer shadow-2xl shadow-sky-600/20"
      >
        {/* Glow effects */}
