...
new Elysian({
  ...
  },
  {
    ...
    code: `import { Agent, ElysianObservability } from "@elysian/core";
import { createModerationScorer } from "@elysian/scorers";
import { openai } from "@ai-sdk/openai";

const observability = new ElysianObservability();
...
const agent = new Agent({
  ...
  },
  {
    ...
    icon: "play",
    image: "/flow-detail-2.png",
    triggerCode: `import { Elysian, createTriggers } from "@elysian/core";
...
new Elysian({
  triggers: createTriggers((on) => {
    ...
    actionCode: `import { Agent, createTool } from "@elysian/core";
import { elysianOps } from "@elysian/elysianops";
...
const createAirtableRecord = createTool({
  ...
  execute: async ({ tableName, fields }) => {
    return elysianOps.actions.airtable.createRecord({
...
const agent = new Agent({
  ...
  },
  {
    ...
    label: "RAG",
    icon: "book",
    image: "/agent-list.png",
    code: `import { openai } from "@ai-sdk/openai";
import { Agent, Elysian } from "@elysian/core";
import { ElysianRagRetriever } from "@elysian/core";

const retriever = new ElysianRagRetriever({
...
// console.log(result.context.get("rag.references"));`,
  ...
  },
  {
    ...
    icon: "bell",
    image: "/mlflow-tracing.png",
    code: `import { ElysianOps } from "@elysian/elysianops";

const elysianOps = new ElysianOps();

// Create an alert for high latency
await elysianOps.alerts.create({
...
await elysianOps.alerts.create({
  ...
  },
  {
    ...
    image: "/agent-chat.png",
    code: `import { openai } from "@ai-sdk/openai";
import { Agent, Elysian, ElysianOpsClient } from "@elysian/core";

const elysianOpsClient = new ElysianOpsClient({
  publicKey: process.env.ELYSIAN_PUBLIC_KEY,
  secretKey: process.env.ELYSIAN_SECRET_KEY,
});
...
new Elysian({
  agents: { agent },
  elysianOpsClient: elysianOpsClient,
});`,
    footerText: "Version and update prompts without code changes using a central registry",
    docLink: "/docs/agents/prompts#elysianops-prompt-management",
  },
  {
    id: "guardrails",
    label: "Guardrails",
    icon: "shield",
    image: "/flow-detail-2.png",
    code: `import { Agent, createGuardrail } from "@elysian/core";
import {
  contentModerationGuardrail,
  piiDetectionGuardrail,
  topicRestrictionGuardrail,
} from "@elysian/guardrails";
