...
import { cn } from "@/utils/elysian/index";
    ...
    <div className="break-words text-slate-600 dark:text-[#dcdcdc] leading-normal tracking-tighter">
      {tweet.entities.map((entity, idx) => {
        ...
        switch (entity.type) {
          ...
          case "mention":
            return (
              <span key={key} className="text-sm font-normal no-underline text-sky-500">
  ...
  try {
    ...
    return (
      <a href={tweetUrl} target="_blank" className="no-underline no-decoration" rel="noreferrer">
        <div
          className={cn(
            "relative flex size-full max-w-lg flex-col gap-2 overflow-hidden rounded-lg border p-4 backdrop-blur-md border-slate-200 dark:border-white/10 border-solid bg-white dark:bg-slate-900/50 hover:border-sky-500 transition-colors duration-200 h-[210px]",
            className,
          )}
          {...props}
        >
