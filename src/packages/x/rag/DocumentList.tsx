...
// --- Mock Data ---
const initialData: KnowledgeDocument[] = [
    { id: "1", name: "HR_Policy_2024.pdf", type: "pdf", status: "ready", tokens: 12500, updatedAt: "2024-01-15T10:00:00Z" },
    { id: "2", name: "Onboarding_Guide_v2.docx", type: "docx", status: "indexing", tokens: 0, updatedAt: "2024-01-18T09:30:00Z" },
    { id: "3", name: "Project_Phoenix_Specs.md", type: "md", status: "ready", tokens: 4500, updatedAt: "2024-01-18T08:15:00Z" },
    { id: "4", name: "Competitor_Analysis.txt", type: "txt", status: "error", tokens: 0, updatedAt: "2024-01-17T14:20:00Z" },
];

export function DocumentList({ onSelectDocument }: { onSelectDocument?: (doc: KnowledgeDocument) => void }) {
    const [data, setData] = useState<KnowledgeDocument[]>(initialData);
        ...
        const interval = setInterval(() => {
            setData(prev => prev.map(doc => {
                if (doc.status === "indexing") {
                    // Randomly finish indexing
                    if (Math.random() > 0.7) {
                        return { ...doc, status: "ready", tokens: Math.floor(Math.random() * 5000) + 1000 };
                    }
                }
                return doc;
            }));
    ...
    const columns: ColumnDef<KnowledgeDocument>[] = [
        ...
        },
        {
            ...
            header: "Tokens",
            cell: ({ row }) => {
                const val = row.getValue("tokens") as number;
                return val > 0 ? <span className="font-mono text-xs">{val.toLocaleString()}</span> : <span className="text-slate-300">-</span>;
            }
        },
        {
                    ...
                    >
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span className="text-xs text-slate-500 uppercase">{doc.type} • {new Date(doc.updatedAt).toLocaleDateString()}</span>
                                </div>
                            </div>
                            <Button
                                variant="ghost"
                        ...
                        <div className="flex items-center justify-between border-t border-slate-50 pt-3 mt-2">
                            ...
                            <span className="text-xs font-mono text-slate-400">
                                {doc.tokens > 0 ? `${doc.tokens.toLocaleString()} tokens` : '-'}
                            </span>
                        </div>
                    </div>
                ))}
