...
export async function sendMessage(payload: SendMessagePayload): Promise<ChatMessage> {
    ...
    const generateDemoReply = (userText: string): ChatMessage => {
        ...
        if (DEMO_MESSAGES[payload.conversationId]) {
            DEMO_MESSAGES[payload.conversationId].push(msg);
        } else {
            DEMO_MESSAGES['session-demo-01'].push(msg);
        }
    ...
    if (isDemo) {
        return generateDemoReply(payload.content);
    }
