...
async function getPresignedURL(
    ...
    const res = await fetch(
        `${API_BASE}/documents/presign?filename=${encodeURIComponent(filename)}`,
        {
            headers: {
    ...
    if (!res.ok) {
        if (res.status === 401) {
            // Force clear auth state if token is invalid/expired
            if (typeof window !== 'undefined') {
                localStorage.removeItem('elysian-auth-storage');
...
export async function updateDocumentText(
    ...
    const res = await fetch(`${API_BASE}/documents/${documentId}/text`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${authToken}`,
...
export async function getDocumentRaw(
    ...
    const res = await fetch(`${API_BASE}/documents/${documentId}/raw`, {
        headers: {
            Authorization: `Bearer ${authToken}`,
            'X-Tenant-ID': tenantId,
        },
